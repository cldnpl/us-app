...
const mediaCols = `id, couple_id, uploader_id, kind, file_path, thumb_path, caption, content_type, size_bytes, is_widget_photo, created_at`

func scanMedia(row pgx.Row) (Media, error) {
	var m Media
	err := row.Scan(&m.ID, &m.CoupleID, &m.UploaderID, &m.Kind, &m.FilePath, &m.ThumbPath,
		&m.Caption, &m.ContentType, &m.SizeBytes, &m.IsWidgetPhoto, &m.CreatedAt)
...
func (s *Store) CreateMedia(ctx context.Context, p CreateMediaParams) (Media, error) {
	return scanMedia(s.pool.QueryRow(ctx,
		`INSERT INTO media (id, couple_id, uploader_id, kind, file_path, thumb_path, caption, content_type, size_bytes)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		 RETURNING `+mediaCols,
		p.ID, p.CoupleID, p.UploaderID, p.Kind, p.FilePath, p.ThumbPath, p.Caption, p.ContentType, p.SizeBytes))
}

func (s *Store) ListMedia(ctx context.Context, coupleID string, limit, offset int) ([]Media, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT `+mediaCols+` FROM media WHERE couple_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
